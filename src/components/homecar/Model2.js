...
const ModelForm = (props) => {
  ...
  function dataDate() {
    fetch(`https://almutamayizun.videosep.com/api/services_dates/${id}`)
      .then((res) => res.json())
      .then((data) => {
        setdateDay(data.data);
        setdatayear(data.data);
        setdatamonth(data.data);
        // console.log(datamonth , datayear , dateDay);
      });
  }
                ...
                {Review === "Add Review" ? (
                  <div className="div-addreview">
                    <form onSubmit={formSubmit}>
                      ...
                      {errorlogin && <span className="errormodel">{props.langs === "ar" ? "please login" : "برجاء تسجيل الدخول"}</span>}
                      <button>
            ...
            <div className="model">
              <div className="modelAbout">
                ...
                <div className="schedule">
                  {props.langs === "ar" ? <h5>Schedule</h5> : <h5>جدول</h5>}
                  {datayear.length > 0 ? (
                    <>
                      <div className="month">
                        ...
                        <h6 style={{ marginLeft: "10px", marginBottom: "0px" }}>
                          {props.langs === "ar"
                            ? datamonth[0].month_name_english
                            : datamonth[0].month_name_arabic}
                          , {datayear[0].year}
                        </h6>
                      </div>
                      ...
                      {hourss.length > 0 ? (
                        <div style={{ marginTop: "40px" }} className="times">
                          {hourss.map((itme) => (
                            <>
                              <NavLink
                                className={`time ${
